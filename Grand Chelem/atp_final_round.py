...
# Fonction qui récupère le nom des deux finalistes si c'est un grand chelem
# Sinon elle renvoie une liste contenant pour seul élément le gagnant du tournoi
...
# Fonction spécifique au grand chelem (les seuls à avoir un septième round)
# Fonction qui renvoie une liste contenant pour seul élément le gagnant du tournoi
...
# Fonction spécifique au grand chelem (les seuls à avoir un septième round)
# Fonction qui renvoie une liste contenant pour seul élément le score de la finale du tournoi
...
while l < len(tournamentList):
    ...
    # URL du drawing pour un tournoi une année arbitrairement choisie juste pour récupérer des données qui ne change pas
    ...
    # Création d'un fichier spécifique au round final du tournoi
    ...
    mon_fichier.write('{ANNEE:<15} {PRIZEMONEY:<15} {TOUR:<15} {RANK1:<5} {JOUEUR1:<25} {RANK2:<5} {JOUEUR2:<25} {GAGNANT:<25} {SCORE}'.format(
        ANNEE="ANNEE",
    ...
    for tournamentYear_url in tournamentYear_urls:
        ...
        while i < len(get_player_name_seventh_round_winner(soup)):
            player1 = get_player_name_sixth_round_winner(soup)[j] #
            player2 = get_player_name_sixth_round_winner(soup)[j+1]
            winner = get_player_name_seventh_round_winner(soup)[0]
            score = get_player_score_seventh_round_winner(soup)[0]
            prize = get_tournament_prizemoney(soup)
            rank1 = get_player_specific_rank(soup2, soup3, player1)
            rank2 = get_player_specific_rank(soup2, soup3, player2)
            mon_fichier.write('{tournamentYear:<15} {Prize:<15} {Round:<15} {Rank1:<5} {Player1:<25} {Rank2:<5} {Player2:<25} {Gagnant:<25} {Score}'.format(
                #tournamentYear=str(tournamentYear),
                tournamentYear=get_tournament_date(soup)[0],
                Prize=prize, 
                Round=str("Final Round"),
                Rank1=rank1,
                Player1=player1,
                Rank2=rank2,
                Player2=player2, 
