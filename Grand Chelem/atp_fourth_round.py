...
def get_player_score_fourth_round_winner(soup):
    ...
    list_score_fourth_round = []
    for score in scores:
        list_score_fourth_round.append(score.a.string)
    return list_score_fourth_round
...
# Création du fichier
mon_fichier = open(get_tournament_title(soup)+"_Fourth_Round", "w")
...
for tournamentYear_url in tournamentYear_urls:
    ...
    while i < len(get_player_name_fourth_round_winner(soup)):
        winner = get_player_name_fourth_round_winner(soup)[n]
        score = get_player_score_fourth_round_winner(soup)[n]
        n+=1
        
        mon_fichier.write('{tournamentYear:<10} {Round:<15} {Player1:<25} {Player2:<25} {Gagnant:<25} {Score}'.format(tournamentYear=str(tournamentYear), Round=str("Fourth Round"), Player1=get_player_name_third_round_winner(soup)[j], Player2=get_player_name_third_round_winner(soup)[j+1], Gagnant=winner, Score=score))
        mon_fichier.write("\n")        
        j+=2
        i+=1
    tournamentYear+=1
...
myfile = get_tournament_title(soup)+"/"+get_tournament_title(soup)+"_Fourth_Round"

if os.path.isfile(myfile):
    os.remove(myfile)

# On déplace le fichier produit dans le dossier du tournoi
shutil.move(get_tournament_title(soup)+"_Fourth_Round", get_tournament_title(soup)+"/")
