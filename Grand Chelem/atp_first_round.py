...
def get_first_round_winner(soup):
    colonne2 = soup.find("td", "col_2")
...
while l < len(tournamentList):
    ...
    for tournamentYear_url in tournamentYear_urls:
        ...
        first_round_player = get_player_name_first_round(soup)

        # Le nombre de gagnant étant égale au nombre de match jouer, on boucle sur ce nombre qui sera le nombre de lignes produites
        while i < len(get_first_round_winner(soup)[0]):
            player1 = first_round_player[j] #
            player2 = first_round_player[j+1]
            winner = get_first_round_winner(soup)[1][i]
            score = get_first_round_winner(soup)[0][i]
