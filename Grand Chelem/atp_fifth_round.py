...
for tournamentYear_url in tournamentYear_urls:
    ...
    while i < len(get_player_name_fifth_round_winner(soup)):
        ...
        mon_fichier.write('{tournamentYear:<10} {Round:<15} {Player1:<25} {Player2:<25} {Gagnant:<25} {Score}'.format(
            tournamentYear=str(tournamentYear), 
            Round=str("Fifth Round"), 
            Player1=get_player_name_fourth_round_winner(soup)[j], 
            Player2=get_player_name_fourth_round_winner(soup)[j+1], 
            Gagnant=winner, 
            Score=score))
        mon_fichier.write("\n")        
        j+=2
